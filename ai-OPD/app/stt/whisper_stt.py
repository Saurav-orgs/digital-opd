"""Speech-to-text via faster-whisper — one of two providers.

Selected with STT_PROVIDER=whisper. See `app/stt/__init__.py` for the
dispatcher and `sarvam_stt.py` for the hosted alternative that is the default.
Nothing in this file changed when Sarvam arrived, deliberately: switching back
has to restore the old behaviour exactly, not an approximation of it.
